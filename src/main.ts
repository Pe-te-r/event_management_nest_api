...
This API provides comprehensive **event management** functionalities, including:  
### **Core Features**  
- 🎟️ **Event Creation & Listing** – Create, update, and browse events  
- 📅 **Schedule Management** – Set dates, times, and recurring events  
- 🎪 **Venue & Ticket Management** – Manage locations, seating, and ticket sales  
- 👥 **Attendee Management** – Handle registrations, check-ins, and attendee data  
- 🔐 **Role-Based Access** – Organizers, admins, attendees(user) and guest have different permissions  
### **Key Endpoints**  
- \`/events\` – Manages event endpoints
- \`/registration\` – Manage registration endpoints
- \`/feedback\` – Manage feedback endpoints
- \`/payments\` – Manage payment endpoints
- \`/users\` – Manage users endpoints
### **Notes**  
- **Authentication required** for most endpoints (JWT Bearer Token).  
- **Admin/Organizer roles** needed for modifying events.  
`)
// - Pagination supported on list endpoints (e.g., \`/events?page=1&limit=10\`).  
